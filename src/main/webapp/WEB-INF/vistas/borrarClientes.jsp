...
<h3>Borrar Clientes</h3>
	<div class="container">
		<div class="form">
			<form action="http://localhost:8080/tienda/clientes/formularioborrarclientes" id="formulario"
				method="POST">
...
<h3>Resultados de la búsqueda</h3>
		<c:forEach items="${listaClientes}" var="clientes">
			...
			<form action="http://localhost:8080/tienda/clientes/borrarclientes"
				method="POST">
				<label>ID Cliente:</label> <input type="text" id="idClientes" name="idClientes" readonly value="${clientes.idClientes}"><br>
				<label>Nombre:</label> <input type="text" id="nombre" name="nombre" value="${clientes.nombre}" readonly><br>
				<label>Correo Electrónico:</label> <input type="text" id="correoElectronico" name="correoElectronico" value="${clientes.correoElectronico}" readonly><br>
				<label for="poblacion">Población:</label><br>
				<select name="idPoblacion" id="idPoblacion" disabled="disabled">
					<c:forEach items="${combosPoblacion}" var="poblacion">
						<option value="${poblacion.id}">${poblacion.nombre}</option>
					</c:forEach>
